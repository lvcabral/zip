 ...
 */
export class ZipFS extends Readonly(FileSystem) {
    label;
    data;
    lazy;
    caseFold;
    ...
    async ready() {
        ...
        while (ptr < cdEnd) {
            const cd = await FileEntry.from(this.data, ptr);
            if (!this.lazy)
                await cd.loadContents();
    ...
    constructor(label, data, lazy = false, caseFold) {
    ...
    _caseFold(original, update = false) {
        if (this.caseFold) {
...
const _isShared = (b) => typeof b == 'object' && b !== null && b.constructor.name === 'SharedArrayBuffer';
export function fromStream(stream, size) {
...
const _Zip = {
    ...
    create(opt) {
        return new ZipFS(opt.name ?? '', getSource(opt.data), opt.lazy, opt.caseFold);
    },
};
